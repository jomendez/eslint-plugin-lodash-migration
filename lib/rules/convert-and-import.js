        ...
        if (node && node.callee && node.callee.object && node.callee.object.name === '_' && node.callee.property.name === 'isArray') {
          ...
          context.report({
            ...
            fix(fixer) {
              ...
              fixerArray.push(
                fixer.replaceTextRange([start, node.range[1]], `typeof ${node.arguments[0].name} ${determineOperator(node)}== 'string'`)
              );
              return [
                ...fixerArray
              ]
          ...
          context.report({
            ...
            fix(fixer) {
              const start = determineOperator(node) === '!' ? node.range[0] - 1 : node.range[0];
              fixerArray.push(
                fixer.replaceTextRange([start, node.range[1]], `${node.arguments[0].name} ${determineOperator(node)}== undefined`)
              );
              return [
                ...fixerArray
              ]
